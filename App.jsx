...
const C = {
  // Backgrounds & surfaces — blanco 50%, café el resto de superficies
  bg:      "#FAF7F4",       // blanco cálido marfil
  s1:      "#FFFFFF",       // blanco puro — cards principales
  s2:      "#F5EDE8",       // café muy claro — superficie secundaria
  s3:      "#E8D8CF",       // café suave — bordes, dividers
  s4:      "#D4B8AC",       // café medio — bordes activos
  // Accent dorado — 5%
  accent:  "#C9A84C",       // dorado cálido
  accentD: "#A07830",       // dorado oscuro
  accentL: "#E8C97A",       // dorado claro
  accentS: "#C9A84C1A",     // dorado wash
  // Rosa pastel — 15%
  pink:    "#F2C4CE",       // rosa pastel principal
  pinkD:   "#E8A0B0",       // rosa medio
  pinkL:   "#FAE8ED",       // rosa muy suave
  pinkS:   "#F2C4CE22",     // rosa wash
  // Negro — 20% (texto)
  t1:      "#1A1209",       // negro cálido
  t2:      "#5C3D2E",       // café oscuro texto
  t3:      "#9C7B6A",       // café medio texto / muted
  // Funcionales
  purple:  "#8B5E3C",       // café rojo (sub-accent)
  purpleS: "#8B5E3C18",
  success: "#7CAE7A",       // verde oliva suave
  warn:    "#C9A84C",       // reutiliza dorado
  info:    "#7A9EC9",
  bear:    "#C9A484",
  bearD:   "#A07848",
  bearL:   "#E8C9A4",
  ...
  useEffect(() => {
    ...
    s.textContent = `
      ...
      *, *::before, *::after { box-sizing: border-box; -webkit-tap-highlight-color: transparent; -webkit-font-smoothing: antialiased; }
      ...
      @keyframes glowRing { 0%,100%{box-shadow:0 0 0 0 rgba(201,168,76,0.3)} 50%{box-shadow:0 0 0 14px rgba(201,168,76,0)} }
      ...
      * { scrollbar-width: none; }
      input:focus { outline: 1.5px solid #C9A84C; outline-offset: 1px; border-color: #C9A84C !important; }
    `;
...
const Chip = ({ children, color = C.accent, bg, style = {} }) => (
  <div style={{ display: "inline-flex", alignItems: "center", background: bg || `${color}18`, color, borderRadius: 100, padding: "3px 10px", fontSize: 11, fontWeight: 700, fontFamily: FONT, ...style }}>{children}</div>
);
...
const FloralBranch = ({ side="left", style={} }) => (
  <svg width="90" height="160" viewBox="0 0 90 160" style={{ opacity:0.3, ...style }} fill="none">
    {side==="left" ? (
      <>
        <path d="M45 155 Q30 120 20 90 Q10 60 25 30" stroke="#C9A84C" strokeWidth="1.5" strokeLinecap="round" />
        <ellipse cx="18" cy="45" rx="10" ry="6" fill="#F2C4CE" transform="rotate(-30 18 45)" />
        <ellipse cx="12" cy="68" rx="10" ry="6" fill="#FAE8ED" transform="rotate(-45 12 68)" />
        <ellipse cx="22" cy="92" rx="9" ry="5" fill="#F2C4CE" transform="rotate(-20 22 92)" />
        <ellipse cx="35" cy="115" rx="8" ry="5" fill="#FAE8ED" transform="rotate(10 35 115)" />
        <circle cx="16" cy="34" r="5" fill="#C9A84C" opacity="0.5" />
      </>
    ) : (
      <>
        <path d="M45 155 Q60 120 70 90 Q80 60 65 30" stroke="#C9A84C" strokeWidth="1.5" strokeLinecap="round" />
        <ellipse cx="72" cy="45" rx="10" ry="6" fill="#F2C4CE" transform="rotate(30 72 45)" />
        <ellipse cx="78" cy="68" rx="10" ry="6" fill="#FAE8ED" transform="rotate(45 78 68)" />
        <ellipse cx="68" cy="92" rx="9" ry="5" fill="#F2C4CE" transform="rotate(20 68 92)" />
        <ellipse cx="55" cy="115" rx="8" ry="5" fill="#FAE8ED" transform="rotate(-10 55 115)" />
        <circle cx="74" cy="34" r="5" fill="#C9A84C" opacity="0.5" />
      </>
    )}
  </svg>
);
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:10 }}>
      ...
      <div style={{ width:size, height:size, position:"relative", flexShrink:0 }}>
        ...
        {/* Avatar circle */}
        <div style={{ width:size,height:size,borderRadius:"50%",overflow:"hidden",background:C.pinkL,border:`2px solid ${C.accent}50`,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",boxShadow:`0 4px 20px ${C.accent}25` }}>
          {photoURL ? (
        ...
        <div style={{ position:"absolute",bottom:1,right:1,width:size*0.3,height:size*0.3,minWidth:24,minHeight:24,borderRadius:"50%",background:C.accent,border:`2px solid ${C.s1}`,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:`0 2px 8px ${C.accent}50`,pointerEvents:"none",zIndex:1 }}>
...
const TabBar = ({ active, onTab }) => (
  <div style={{ display:"flex",alignItems:"center",height:64,background:"rgba(250,247,244,0.97)",backdropFilter:"blur(20px)",borderTop:`1px solid ${C.s3}`,flexShrink:0,paddingBottom:"env(safe-area-inset-bottom,0px)" }}>
    {TABS.map(t => {
      const isActive = active===t.id;
      return (
        <button key={t.id} className="pressable" onClick={()=>onTab(t.id)}
          style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4,position:"relative",background:"none",border:"none",padding:"10px 0",cursor:"pointer" }}>
          {t.icon(isActive)}
          <span style={{ fontFamily:FONT,fontSize:10,fontWeight:600,color:isActive?C.accent:C.t3 }}>{t.label}</span>
          {isActive && <div style={{ position:"absolute",bottom:0,width:20,height:2,borderRadius:2,background:C.accent }} />}
        </button>
      );
    })}
  </div>
...
const DEFAULT_ROUTINES = [
  {
    ...
    emoji:"🦵", color:C.pink, dark:C.pinkD, duration:50, difficulty:3,
    exercises:[
  ...
  },
  {
    ...
    emoji:"🏋️", color:C.accent, dark:C.accentD, duration:45, difficulty:3,
    exercises:[
  ...
  },
  {
    ...
    emoji:"💪", color:C.bear, dark:C.bearD, duration:25, difficulty:2,
    exercises:[
...
const PhotoLightbox = ({ photo, onClose, onDelete }) => (
  <div className="anim-fadeIn" onClick={onClose}
    style={{ position:"fixed",inset:0,zIndex:400,background:"rgba(26,10,14,0.92)",backdropFilter:"blur(16px)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:FONT }}>
    <div onClick={e=>e.stopPropagation()} className="anim-slideUp"
      ...
      <div style={{ height:300,position:"relative",background: photo.dataURL ? "#000" : `linear-gradient(145deg,${photo.gradA},${photo.gradB})`,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden" }}>
        ...
        <div style={{ position:"absolute",top:12,right:12,width:36,height:36,borderRadius:11,background:"rgba(255,255,255,0.9)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,boxShadow:"0 3px 12px rgba(0,0,0,0.2)",zIndex:1 }}>{photo.emoji}</div>
      </div>
      <div style={{ background:C.s1,padding:"18px 20px" }}>
        ...
        {onDelete && (
          <button className="pressable" onClick={()=>onDelete(photo)}
            style={{ marginTop:14,width:"100%",background:"none",border:`1.5px solid ${C.accent}33`,borderRadius:12,padding:"10px",fontSize:13,fontWeight:700,color:C.accent,cursor:"pointer",fontFamily:FONT }}>
            Eliminar foto
          </button>
        )}
        <button className="pressable" onClick={onClose}
          style={{ marginTop:10,width:"100%",background:C.s2,border:"none",borderRadius:12,padding:"11px",fontSize:14,fontWeight:700,color:C.t2,cursor:"pointer",fontFamily:FONT }}>
          Cerrar
        </button>
      </div>
    </div>
...
const PhotoCard = ({ photo, onTap }) => (
  <div className="pressable anim-fadeUp" onClick={()=>onTap(photo)}
    style={{ borderRadius:16,overflow:"hidden",cursor:"pointer",position:"relative",aspectRatio:"1/1",background:photo.dataURL?"#000":`linear-gradient(145deg,${photo.gradA},${photo.gradB})`,boxShadow:`0 3px 12px ${photo.gradA||"#C8102E"}40` }}>
    {photo.dataURL ? (
      <img src={photo.dataURL} alt={photo.label} style={{ position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover" }}/>
    ) : (
    ...
    <div style={{ position:"absolute",top:8,left:8,width:28,height:28,borderRadius:9,background:"rgba(255,255,255,0.85)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,backdropFilter:"blur(4px)" }}>{photo.emoji}</div>
  ...
  return (
    <div className="anim-fadeIn" style={{ position:"fixed",inset:0,zIndex:300,background:"#000",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:FONT }}>
      ...
      <div style={{ position:"absolute",top:0,left:0,right:0,padding:"20px 22px",paddingTop:"calc(20px + env(safe-area-inset-top,0px))",display:"flex",justifyContent:"space-between",alignItems:"center",zIndex:10 }}>
        <button onClick={onClose} style={{ background:"rgba(255,255,255,0.15)",border:"none",borderRadius:100,padding:"8px 16px",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:FONT,backdropFilter:"blur(8px)" }}>Cancelar</button>
        <div style={{ fontSize:13,fontWeight:600,color:"rgba(255,255,255,0.8)",background:"rgba(0,0,0,0.3)",borderRadius:100,padding:"5px 12px",backdropFilter:"blur(8px)" }}>📍 Gym · Hoy</div>
        {/* Flip button */}
        {!captured && (
          <button onClick={handleFlip} style={{ background:"rgba(255,255,255,0.15)",border:"none",borderRadius:100,width:38,height:38,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",backdropFilter:"blur(8px)" }}>
      ...
      {/* Visor cuadrado */}
      <div style={{ width:"min(88vw,340px)",height:"min(88vw,340px)",borderRadius:28,overflow:"hidden",position:"relative",border:"2px solid rgba(255,255,255,0.18)",boxShadow:"0 0 60px rgba(200,16,46,0.35)" }}>
        {/* Video live */}
        <video
          ref={videoRef}
          playsInline
        ...
        {!ready && !captured && !camError && (
          <div style={{ position:"absolute",inset:0,background:"#0d0007",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14 }}>
        ...
        {/* Error de permisos */}
        {camError && (
          <div style={{ position:"absolute",inset:0,background:"#0d0007",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:12,padding:24 }}>
        ...
        {captured && (
          <div style={{ position:"absolute",inset:0 }}>
            ...
            <div style={{ position:"absolute",top:12,right:12,width:44,height:44,borderRadius:14,background:"rgba(255,255,255,0.9)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,boxShadow:"0 4px 16px rgba(0,0,0,0.25)" }}>{routineEmoji}</div>
        ...
        {!captured && ready && (
          <>
            <div style={{ position:"absolute",top:12,left:12,width:22,height:22,borderTop:"2px solid rgba(255,255,255,0.7)",borderLeft:"2px solid rgba(255,255,255,0.7)",borderRadius:"6px 0 0 0",pointerEvents:"none" }}/>
            <div style={{ position:"absolute",top:12,right:12,width:22,height:22,borderTop:"2px solid rgba(255,255,255,0.7)",borderRight:"2px solid rgba(255,255,255,0.7)",borderRadius:"0 6px 0 0",pointerEvents:"none" }}/>
            <div style={{ position:"absolute",bottom:12,left:12,width:22,height:22,borderBottom:"2px solid rgba(255,255,255,0.7)",borderLeft:"2px solid rgba(255,255,255,0.7)",borderRadius:"0 0 0 6px",pointerEvents:"none" }}/>
            <div style={{ position:"absolute",bottom:12,right:12,width:22,height:22,borderBottom:"2px solid rgba(255,255,255,0.7)",borderRight:"2px solid rgba(255,255,255,0.7)",borderRadius:"0 0 6px 0",pointerEvents:"none" }}/>
          </>
        )}
      </div>
      ...
      <div style={{ position:"absolute",bottom:0,left:0,right:0,padding:"28px 40px",paddingBottom:"calc(28px + env(safe-area-inset-bottom,0px))",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
        ...
        <div style={{ width:52,height:52,borderRadius:14,overflow:"hidden",border:"2px solid rgba(255,255,255,0.25)",background:"rgba(255,255,255,0.1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0 }}>
          {routineEmoji}
        </div>
        ...
        {!captured ? (
          <button
            ...
            style={{ width:78,height:78,borderRadius:"50%",background:"transparent",border:"4px solid rgba(255,255,255,0.5)",cursor:(countdown!==null||!ready)?"not-allowed":"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:`0 0 0 2px rgba(255,255,255,0.2),0 8px 32px rgba(200,16,46,0.5)`,flexShrink:0,opacity:(countdown!==null||!ready)?0.5:1,transition:"opacity 0.2s,transform 0.1s",padding:0 }}
          >
          ...
          <button className="pressable" onClick={handleSave}
            style={{ background:C.accent,border:"none",borderRadius:100,padding:"16px 32px",color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:FONT,boxShadow:`0 8px 28px ${C.accent}70` }}>
            Guardar foto 💾
          </button>
        )}
        ...
        {/* Retake */}
        {captured ? (
          <button onClick={()=>setCaptured(null)} style={{ width:52,height:52,borderRadius:14,background:"rgba(255,255,255,0.12)",border:"none",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",backdropFilter:"blur(8px)" }}>
...
const RoutinePickerModal = ({ routines, current, onSelect, onClose }) => {
  ...
  return (
    <div className="anim-fadeIn" style={{ position:"fixed",inset:0,zIndex:250,background:"rgba(26,10,14,0.6)",backdropFilter:"blur(10px)",display:"flex",alignItems:"flex-end",justifyContent:"center",fontFamily:FONT }} onClick={onClose}>
      <div className="anim-slideUp"
        ...
        <div style={{ width:36,height:4,borderRadius:2,background:C.s3,margin:"0 auto 18px",padding:"0 20px" }}/>
            ...
            return (
              <div key={r.id}
                ...
                style={{
                  ...
                  background:C.bg,borderRadius:22,padding:"20px 18px",textAlign:"center",
                ...
                <div style={{ width:56,height:56,borderRadius:18,background:`${r.color}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,margin:"0 auto 14px" }}>{r.emoji}</div>
                ...
                <div style={{ textAlign:"left" }}>
                  ...
                  <div style={{ display:"flex",gap:4 }}>
                    {[1,2,3,4,5].map(d=>(
                      <div key={d} style={{ height:20,flex:1,borderRadius:5,background:d<=r.difficulty?r.color:C.s3 }}/>
                    ))}
                  </div>
                </div>
                {current?.id===r.id&&(
        ...
        <div style={{ display:"flex",justifyContent:"center",gap:6,marginTop:16,marginBottom:20 }}>
          {routines.map(r=>(
            <div key={r.id} onClick={()=>goToCard(r.id)} className="pressable"
              style={{ width:pickedId===r.id?16:6,height:6,borderRadius:3,background:pickedId===r.id?pickedRoutine.color:C.s3,transition:"all 0.25s cubic-bezier(.22,1,.36,1)" }}/>
          ))}
        </div>
...
const StartWorkoutModal = ({ routine, onConfirm, onClose }) => {
  ...
  return (
    <div className="anim-fadeIn" onClick={onClose}
      style={{ position:"fixed",inset:0,zIndex:250,background:"rgba(26,10,14,0.55)",backdropFilter:"blur(10px)",display:"flex",alignItems:"flex-end",justifyContent:"center",fontFamily:FONT }}>
      <div
        ref={sheetRef}
        onClick={e=>e.stopPropagation()}
        ...
        style={{ width:"100%",maxWidth:480,background:C.s1,borderRadius:"28px 28px 0 0",padding:"0 24px 24px",paddingBottom:"calc(24px + env(safe-area-inset-bottom,0px))",border:`1px solid ${C.s3}`,boxShadow:`0 -8px 40px rgba(201,168,76,0.15)`,touchAction:"none" }}>
        {/* Drag handle */}
        <div style={{ padding:"14px 0 10px",display:"flex",justifyContent:"center",cursor:"grab" }}>
          <div style={{ width:36,height:4,borderRadius:2,background:C.s4 }}/>
        </div>
        <div style={{ textAlign:"center",marginBottom:20 }}>
          <div style={{ width:64,height:64,borderRadius:20,background:`${routine.color}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:32,margin:"0 auto 14px" }}>{routine.emoji}</div>
          <div style={{ fontSize:20,fontWeight:900,color:C.t1,letterSpacing:"-0.3px",fontFamily:FONT_DISPLAY }}>{routine.name}</div>
          <div style={{ fontSize:13,color:C.t2,marginTop:4 }}>{routine.duration} min · {routine.exercises.length} ejercicios</div>
        </div>
        <div style={{ display:"flex",gap:10,marginBottom:16 }}>
          {[
            { icon:"⏱", label:"Duración", val:`${routine.duration} min` },
            { icon:"📋", label:"Ejercicios", val:routine.exercises.length },
            { icon:"🔥", label:"Dificultad", val:"⭐".repeat(routine.difficulty) },
          ].map((m,i)=>(
            <div key={i} style={{ flex:1,background:C.bg,borderRadius:14,padding:"10px 6px",textAlign:"center",border:`1px solid ${C.s3}` }}>
              <div style={{ fontSize:16 }}>{m.icon}</div>
              <div style={{ fontSize:13,fontWeight:800,color:C.t1,marginTop:4 }}>{m.val}</div>
              <div style={{ fontSize:9,fontWeight:700,color:C.t3,textTransform:"uppercase",letterSpacing:"0.06em",marginTop:2 }}>{m.label}</div>
            </div>
          ))}
        </div>
...
const HomeScreen = ({ onStartWorkout, routines, todayRoutine, onChangeTodayRoutine, onOpenRoutinePicker, onOpenStartConfirm, onOpenLightbox, onGoStats }) => {
  ...
  return (
    <div style={{ flex:1,overflowY:"auto",background:C.bg,fontFamily:FONT,position:"relative" }}>
      {showCamera&&<CameraModal onClose={()=>setShowCamera(false)} onCapture={handleCapture} routineEmoji={todayRoutine?.emoji||"💪"}/>}
      ...
      <div className="anim-fadeUp" style={{ padding:"16px 22px 0",animationDelay:"0.05s" }}>
        <div style={{ position:"relative",background:C.s1,borderRadius:28,padding:"32px 22px 26px",textAlign:"center",border:`1px solid ${C.s3}`,boxShadow:`0 4px 24px rgba(201,168,76,0.10)`,overflow:"hidden" }}>
          <div style={{ position:"absolute",top:-40,right:-40,width:120,height:120,borderRadius:"50%",background:`radial-gradient(circle,${C.pinkL} 0%,transparent 70%)`,pointerEvents:"none" }}/>
          <div style={{ fontSize:13,fontWeight:600,color:C.accent,letterSpacing:"0.06em",marginBottom:20,fontFamily:FONT_DISPLAY }}>♥ Más fuertes juntos, cada día ♥</div>
          ...
          <button className="pressable" onClick={onOpenStartConfirm}
            style={{ background:`linear-gradient(135deg,${C.accent},${C.accentD})`,border:"none",borderRadius:16,padding:"14px 32px",fontSize:14,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:FONT,boxShadow:`0 6px 18px ${C.accent}35`,marginBottom:18 }}>
            💪 Empezar entrenamiento
          </button>

          {/* Routine selector pill */}
          <div className="pressable" onClick={onOpenRoutinePicker} style={{ display:"inline-flex",alignItems:"center",gap:10,background:C.s2,borderRadius:100,padding:"10px 18px",border:`1px solid ${C.s3}`,cursor:"pointer" }}>
            <span style={{ fontSize:16 }}>{todayRoutine.emoji}</span>
            <div style={{ textAlign:"left" }}>
      ...
      <div className="anim-fadeUp pressable" onClick={onGoStats} style={{ padding:"22px 22px 0",animationDelay:"0.15s" }}>
        ...
        <div style={{ background:C.s1,borderRadius:20,padding:"18px",border:`1px solid ${C.s3}` }}>
          {/* Mini bar chart */}
          <div style={{ display:"flex",gap:5,alignItems:"flex-end",height:48,marginBottom:14 }}>
            {STATS_WEEK.map((v,i)=>{
              const max=Math.max(...STATS_WEEK);
              const h=max>0?(v/max)*40:3;
              const isToday=i===2;
              return (
                <div key={i} style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4 }}>
                  <div style={{ width:"100%",height:h||3,borderRadius:4,minHeight:3,background:isToday?`linear-gradient(180deg,${C.accent},${C.accentD})`:v>0?C.s4:C.s2,transition:"height 0.5s" }}/>
      ...
      <div ref={historialRef} className="anim-fadeUp" style={{ padding:"22px 22px 0",animationDelay:"0.2s" }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16 }}>
          ...
          <button className="pressable" onClick={()=>setShowCamera(true)} style={{ display:"flex",alignItems:"center",gap:6,background:C.accent,border:"none",borderRadius:100,padding:"8px 16px",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:FONT }}>
      ...
      <div style={{ position:"fixed",bottom:88,left:"50%",transform:`translateX(-50%) translateY(${cameraFabVisible?0:20}px)`,opacity:cameraFabVisible?1:0,transition:"all 0.35s cubic-bezier(.34,1.56,.64,1)",zIndex:150,pointerEvents:cameraFabVisible?"auto":"none" }}>
        <button className="pressable" onClick={()=>setShowCamera(true)} style={{ display:"flex",alignItems:"center",gap:10,background:C.accent,border:"none",borderRadius:100,padding:"13px 24px",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:FONT,boxShadow:`0 8px 28px ${C.accent}55`,whiteSpace:"nowrap" }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
            <path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            <circle cx="12" cy="13" r="4" stroke="#fff" strokeWidth="2"/>
          </svg>
          Subir foto
          <svg width="14" height="14" viewBox="0 0 24 24" fill="#fff" style={{ animation:"heartBeat 2s ease-in-out infinite" }}>
...
const EditRoutineModal = ({ routine, onSave, onClose }) => {
  ...
  const inputStyle = { width:"100%",background:C.s2,border:`1px solid ${C.s3}`,borderRadius:12,padding:"10px 14px",fontSize:14,color:C.t1,fontFamily:FONT,outline:"none" };
  const smallInput = { ...inputStyle, padding:"8px 10px",fontSize:13,textAlign:"center" };

  return (
    <div className="anim-fadeIn" style={{ position:"fixed",inset:0,zIndex:260,background:"rgba(26,10,14,0.6)",backdropFilter:"blur(10px)",display:"flex",alignItems:"flex-end",justifyContent:"center",fontFamily:FONT }} onClick={onClose}>
      <div className="anim-slideUp" onClick={e=>e.stopPropagation()} style={{ width:"100%",maxWidth:480,overflowY:"auto",background:C.bg,borderRadius:"28px 28px 0 0",maxHeight:"92vh",paddingBottom:"env(safe-area-inset-bottom,0px)" }}>
        <div style={{ position:"sticky",top:0,background:C.bg,padding:"16px 20px 14px",borderBottom:`1px solid ${C.s3}`,display:"flex",justifyContent:"space-between",alignItems:"center",zIndex:1 }}>
          ...
          <button onClick={handleSave} style={{ background:C.accent,border:"none",borderRadius:100,padding:"7px 16px",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:FONT }}>Guardar</button>
        </div>
        ...
        <div style={{ padding:"16px 20px" }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20 }}>
            ...
            <div>
              ...
              <div style={{ display:"flex",gap:4,marginTop:4 }}>
                {[1,2,3,4,5].map(d=>(
                  <div key={d} className="pressable" onClick={()=>setDifficulty(d)} style={{ height:28,flex:1,borderRadius:6,background:d<=difficulty?routine.color:C.s3,cursor:"pointer",transition:"background 0.2s" }}/>
                ))}
              </div>
            </div>
          </div>
          ...
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12 }}>
            ...
            <button className="pressable" onClick={()=>setShowAddEx(v=>!v)} style={{ display:"flex",alignItems:"center",gap:5,background:C.accent,border:"none",borderRadius:100,padding:"5px 12px",color:"#fff",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:FONT }}>
              <span style={{ fontSize:16,lineHeight:1 }}>+</span> Agregar
            </button>
          </div>

          {showAddEx&&(
            <div className="anim-fadeUp" style={{ background:C.s2,borderRadius:18,padding:"14px",marginBottom:14,border:`1px solid ${C.s3}` }}>
              ...
              <div style={{ display:"flex",gap:8 }}>
                <button onClick={()=>setShowAddEx(false)} style={{ flex:1,background:C.s1,border:`1px solid ${C.s3}`,borderRadius:12,padding:"10px",fontSize:13,fontWeight:600,color:C.t2,cursor:"pointer",fontFamily:FONT }}>Cancelar</button>
                <button className="pressable" onClick={addEx} style={{ flex:2,background:C.accent,border:"none",borderRadius:12,padding:"10px",fontSize:13,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:FONT }}>Añadir ejercicio</button>
              </div>
            </div>
          )}

          {exercises.map((ex,i)=>(
            <div key={ex._id} style={{ background:C.s1,borderRadius:16,padding:"12px 14px",marginBottom:10,border:`1px solid ${C.s3}`,display:"flex",alignItems:"center",gap:12 }}>
              <div style={{ width:28,height:28,borderRadius:9,background:C.s2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:800,color:C.t3,flexShrink:0 }}>{i+1}</div>
              ...
              <button className="pressable" onClick={()=>removeEx(ex._id)} style={{ width:28,height:28,borderRadius:9,background:`${C.accent}15`,border:"none",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0 }}>
    ...
    <>
      ...
      <div style={{ flex:1,overflowY:"auto",background:C.bg,fontFamily:FONT }}>
        <div style={{ padding:"16px 22px 0" }}>
          <div style={{ fontSize:24,fontWeight:900,color:C.t1,letterSpacing:"-0.5px",marginBottom:4 }}>Rutinas</div>
          <div style={{ fontSize:14,color:C.t2,marginBottom:20 }}>Escoge y entrena juntos 💑</div>
        </div>
        <div style={{ padding:"0 22px 24px",display:"flex",flexDirection:"column",gap:18 }}>
          {routines.map((r,i)=>(
            <div key={r.id} className="anim-fadeUp pressable" onClick={()=>handleCardClick(r)} style={{ borderRadius:24,overflow:"hidden",background:C.s1,border:`1px solid ${C.s3}`,animationDelay:`${i*0.08}s`,boxShadow:"0 2px 10px rgba(0,0,0,0.04)",cursor:"pointer" }}>
              <div style={{ height:5,background:`linear-gradient(90deg,${r.color},${r.dark})` }}/>
              <div style={{ padding:"20px 20px" }}>
                <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
                  <div style={{ flex:1,minWidth:0 }}>
                    <div style={{ fontSize:11,fontWeight:700,color:r.color,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:4 }}>{r.sub}</div>
                    <div style={{ fontSize:22,fontWeight:900,color:C.t1,letterSpacing:"-0.3px" }}>{r.name}</div>
                  </div>
                  <div style={{ width:52,height:52,borderRadius:16,background:`${r.color}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,flexShrink:0 }}>{r.emoji}</div>
                </div>
                <div style={{ display:"flex",gap:4,marginTop:14,marginBottom:14 }}>
                  {[1,2,3,4,5].map(d=><div key={d} style={{ height:4,flex:1,borderRadius:2,background:d<=r.difficulty?r.color:C.s3 }}/>)}
                </div>
                <div style={{ display:"flex",gap:12,justifyContent:"space-between",alignItems:"center" }}>
                  {[
                    { icon:"⏱",val:`${r.duration} min` },
                    { icon:"📋",val:`${r.exercises.length} ejerc.` },
                  ].map((m,i)=>(
                  ...
                  <div style={{ flex:1 }}/>
                  <button className="pressable" onClick={(e)=>{e.stopPropagation();setEditingRoutine(r);}} style={{ background:C.s2,border:`1px solid ${C.s3}`,borderRadius:10,padding:"7px 13px",fontSize:11,fontWeight:700,color:C.t2,cursor:"pointer",fontFamily:FONT,display:"flex",alignItems:"center",gap:4 }}>
                  ...
                  <button className="pressable" onClick={(e)=>{e.stopPropagation();onSelect(r);}} style={{ background:r.color,border:"none",borderRadius:10,padding:"7px 15px",fontSize:11,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:FONT }}>Empezar</button>
                </div>
              </div>
            </div>
          ))}
  ...
  return (
    <div style={{ flex:1,overflowY:"auto",background:C.bg,fontFamily:FONT,padding:"16px 22px 32px" }}>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:24 }}>
        <div>
          <div style={{ fontSize:24,fontWeight:900,color:C.t1,letterSpacing:"-0.5px" }}>Estadísticas</div>
          <div style={{ fontSize:14,color:C.t2,marginTop:2 }}>Tu progreso juntos 📈</div>
        </div>
        <div style={{ display:"flex",background:C.s2,borderRadius:10,padding:3,gap:2 }}>
          {["week","month"].map(p=>(
            <button key={p} onClick={()=>setPeriod(p)}
              style={{ background:period===p?C.s1:"transparent",border:"none",borderRadius:8,padding:"5px 12px",fontSize:11,fontWeight:700,color:period===p?C.t1:C.t3,cursor:"pointer",fontFamily:FONT,transition:"all 0.18s",boxShadow:period===p?"0 1px 4px rgba(0,0,0,0.08)":"none" }}>
              {p==="week"?"Semana":"Mes"}
            </button>
          ))}
        </div>
      ...
      {/* Activity chart */}
      <div className="anim-fadeUp" style={{ background:C.s1,borderRadius:22,padding:"18px",marginBottom:14,border:`1px solid ${C.s3}` }}>
        ...
        <div style={{ display:"flex",gap:period==="week"?8:3,alignItems:"flex-end",height:80 }}>
          {data.map((v,i)=>{
            const h = max>0?(v/max)*64:3;
            ...
            return (
              <div key={i} className={clickable?"pressable":""} onClick={()=>{ if(clickable) setSelectedDay(i===selectedDay?null:i); }}
                style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:5,cursor:clickable?"pointer":"default" }}>
                {v>0&&<span style={{ fontSize:8,fontWeight:700,color:isSelected?C.accent:C.t3,opacity:isSelected?1:0.7 }}>{v}m</span>}
                <div style={{
                  width:"100%",height:h||3,borderRadius:6,minHeight:3,
                  background: isSelected
                    ? `linear-gradient(180deg,${C.accentL},${C.accent})`
                    : isToday
                    ? `linear-gradient(180deg,${C.accent},${C.accentD})`
                    : v>0?C.s4:C.s2,
                  transition:"all 0.35s cubic-bezier(.22,1,.36,1)",
                  transform: isSelected?"scaleY(1.05)":"scaleY(1)",
                  transformOrigin:"bottom",
                  boxShadow: isSelected?`0 4px 12px ${C.accent}40`:"none",
                }}/>
      ...
      {[
        ...
        <div key={i} className="anim-fadeUp" style={{ background:C.s1,borderRadius:20,padding:"16px",marginBottom:10,border:`1px solid ${C.s3}`,animationDelay:`${i*0.07}s`,boxShadow:`0 2px 12px ${s.color}10` }}>
          ...
          <div style={{ height:4,borderRadius:2,background:C.s3,overflow:"hidden" }}>
            <div style={{ height:"100%",borderRadius:2,background:s.color,width:`${s.pct*100}%`,transition:"width 0.8s cubic-bezier(.22,1,.36,1)" }}/>
          </div>
      ...
      {selectedDay!==null && (
        <div className="anim-fadeIn" onClick={()=>setSelectedDay(null)}
          style={{ position:"fixed",inset:0,zIndex:250,background:"rgba(26,10,14,0.45)",backdropFilter:"blur(8px)",display:"flex",alignItems:"flex-end",justifyContent:"center",fontFamily:FONT }}>
          <div
            ref={sheetRef}
            className="anim-slideUp"
            onClick={e=>e.stopPropagation()}
            ...
            onTouchMove={onTouchMove}
            onTouchEnd={onTouchEnd}
            style={{ width:"100%",maxWidth:480,background:C.s1,borderRadius:"26px 26px 0 0",paddingBottom:"calc(28px + env(safe-area-inset-bottom,0px))",border:`1px solid ${C.s3}`,boxShadow:`0 -4px 32px rgba(201,168,76,0.12)`,touchAction:"none" }}>
            {/* Drag handle */}
            ...
            <div style={{ padding:"4px 22px 20px" }}>
              <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:18 }}>
                <div style={{ width:44,height:44,borderRadius:14,background:`${C.accent}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22 }}>📅</div>
                <div>
              ...
              {DAY_EXERCISES[selectedDay].exercises.length > 0 ? (
                <>
                  ...
                  {DAY_EXERCISES[selectedDay].exercises.map((ex,i)=>(
                    <div key={i} style={{ display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:12,background:i%2===0?C.s2:C.bg,marginBottom:6 }}>
...
const EditProfileModal = ({ profile, onSave, onClose }) => {
  ...
  const inputStyle={ width:"100%",background:C.s2,border:`1px solid ${C.s3}`,borderRadius:12,padding:"10px 14px",fontSize:14,color:C.t1,fontFamily:FONT,outline:"none" };
  return (
    <div className="anim-fadeIn" style={{ position:"fixed",inset:0,zIndex:260,background:"rgba(26,10,14,0.6)",backdropFilter:"blur(10px)",display:"flex",flexDirection:"column",fontFamily:FONT }}>
      <div className="anim-slideUp" style={{ background:C.bg,marginTop:"auto",borderRadius:"28px 28px 0 0",maxHeight:"85vh",overflowY:"auto",paddingBottom:"env(safe-area-inset-bottom,0px)" }}>
        <div style={{ position:"sticky",top:0,background:C.bg,padding:"16px 20px 14px",borderBottom:`1px solid ${C.s3}`,display:"flex",justifyContent:"space-between",alignItems:"center",zIndex:1 }}>
          ...
          <button onClick={()=>{onSave({name,partner,emoji});onClose();}} style={{ background:C.accent,border:"none",borderRadius:100,padding:"7px 16px",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:FONT }}>Guardar</button>
        </div>
        <div style={{ padding:"20px" }}>
          <Label style={{ marginBottom:10 }}>Avatar</Label>
          <div style={{ display:"flex",gap:10,marginBottom:20,flexWrap:"wrap" }}>
            {avatarOptions.map(e=>(
              <div key={e} className="pressable" onClick={()=>setEmoji(e)} style={{ width:44,height:44,borderRadius:14,background:emoji===e?`${C.accent}20`:C.s2,border:`2px solid ${emoji===e?C.accent:C.s3}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,cursor:"pointer",transition:"all 0.15s" }}>{e}</div>
            ))}
          </div>
          <Label style={{ marginBottom:8 }}>Nombre</Label>
...
const ProfileScreen = ({ profile, onEditProfile }) => {
  ...
  <div style={{ flex:1,overflowY:"auto",background:C.bg,fontFamily:FONT }}>
    <div style={{ position:"relative",padding:"28px 22px 24px",background:`linear-gradient(160deg,${C.s2} 0%,${C.bg} 100%)`,textAlign:"center",borderBottom:`1px solid ${C.s3}` }}>
      ...
      <div style={{ width:80,height:80,borderRadius:26,margin:"0 auto 12px",background:`linear-gradient(135deg,${C.s3},${C.s2})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:38,border:`3px solid ${C.accent}40`,boxShadow:`0 0 0 6px ${C.accent}12` }}>{profile.emoji}</div>
      ...
      <div style={{ display:"flex",gap:10,justifyContent:"center",marginTop:14 }}>
        <button className="pressable" onClick={onEditProfile} style={{ display:"inline-flex",alignItems:"center",gap:6,background:C.s1,border:`1px solid ${C.s3}`,borderRadius:100,padding:"7px 18px",fontSize:12,fontWeight:700,color:C.t2,cursor:"pointer",fontFamily:FONT }}>
    ...
    <div style={{ padding:"16px 22px 32px" }}>
      {[
        ...
        <div key={i} style={{ background:C.s1,borderRadius:16,padding:"14px 16px",marginBottom:8,display:"flex",justifyContent:"space-between",border:`1px solid ${C.s3}` }}>
...
const ExerciseRow = ({ ex, idx, accent, onToggle, style={} }) => {
  ...
  const editableChip = (label, value, setter, type="number") => {
    ...
    return (
      <div
        style={{ flex:1,background:done?`${accent}10`:C.s2,borderRadius:10,padding:"8px 4px",textAlign:"center",transition:"background 0.3s, opacity 0.2s",opacity:unlocked?1:0.85,border:unlocked?`1px solid ${accent}50`:"1px solid transparent" }}
  ...
  if (done && !expanded) {
    return (
      <div
        ...
        <div style={{ width:28,height:28,borderRadius:10,flexShrink:0,background:`linear-gradient(135deg,${accent},${C.accentD})`,display:"flex",alignItems:"center",justifyContent:"center" }}>
    ...
    <div
      ...
      <div style={{ padding:"18px 18px" }}>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:14,marginBottom:16 }}>
          {/* Index / check bubble */}
          <div style={{ width:36,height:36,borderRadius:12,flexShrink:0,background:done?`linear-gradient(135deg,${accent},${C.accentD})`:C.s2,display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.25s" }}>
            {done?(
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:6,marginTop:6,flexWrap:"wrap" }}>
              ...
              <div onClick={e=>{e.stopPropagation();handleChipTap("machine");}} style={{ display:"flex",alignItems:"center",gap:3,background:C.s2,borderRadius:100,padding:"3px 9px",border:unlockedField==="machine"?`1px solid ${accent}50`:"1px solid transparent" }}>
        ...
        <button
          ...
          style={{
            width:"100%",border:"none",borderRadius:14,padding:"11px",
...
const ExerciseScreen = ({ routine, onBack }) => {
  ...
  const inputS={ background:C.s2,border:`1px solid ${C.s3}`,borderRadius:10,padding:"9px 12px",fontSize:14,color:C.t1,fontFamily:FONT,outline:"none",width:"100%" };
    ...
    <div className="anim-slideR" style={{ flex:1,display:"flex",flexDirection:"column",background:C.bg,fontFamily:FONT,overflow:"hidden" }}>
      ...
      <div style={{ padding:"16px 20px 14px",background:C.bg,borderBottom:`1px solid ${C.s3}`,flexShrink:0 }}>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:14 }}>
          <div style={{ width:48,height:48,borderRadius:16,flexShrink:0,background:`${routine.color}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24 }}>{routine.emoji}</div>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:20,fontWeight:900,color:C.t1,letterSpacing:"-0.3px" }}>{routine.name}</div>
        ...
        <div style={{ marginTop:14,height:5,borderRadius:3,background:C.s3,overflow:"hidden" }}>
          <div style={{ height:"100%",borderRadius:3,background:`linear-gradient(90deg,${routine.color},${routine.dark})`,width:`${pct*100}%`,transition:"width 0.5s cubic-bezier(.22,1,.36,1)" }}/>
        </div>
      </div>
      ...
      <div style={{ flex:1,overflowY:"auto",padding:"16px 20px 24px" }}>
        ...
        {showAdd ? (
          <div className="anim-slideDown" style={{ background:C.s1,borderRadius:20,marginBottom:10,border:`1.5px solid ${routine.color}30`,overflow:"hidden" }}>
            ...
            <div style={{ display:"flex",borderBottom:`1px solid ${C.s3}` }}>
              {[["preset","📋 Presets"],["custom","✏️ Nuevo"]].map(([mode,label])=>(
                ...
                <div style={{ maxHeight:260,overflowY:"auto",display:"flex",flexDirection:"column",gap:6 }}>
                  ...
                  {filteredPresets.map((ex,i)=>(
                    <div key={i} className="pressable" onClick={()=>addExercise(ex)}
                      style={{ display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:12,background:C.s2,border:`1px solid ${C.s3}`,cursor:"pointer",transition:"background 0.15s" }}>
                      <div style={{ width:32,height:32,borderRadius:10,background:`${ex._routineColor}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,flexShrink:0 }}>{ex._routineEmoji}</div>
                      <div style={{ flex:1,minWidth:0 }}>
                      ...
                      <div style={{ width:26,height:26,borderRadius:8,background:routine.color,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>
                        <span style={{ color:"#fff",fontSize:16,lineHeight:1,marginTop:-1 }}>+</span>
                      </div>
                    </div>
                  ))}
                </div>
                <button onClick={()=>setShowAdd(false)} style={{ marginTop:12,width:"100%",background:C.s2,border:`1px solid ${C.s3}`,borderRadius:12,padding:"10px",fontSize:13,fontWeight:600,color:C.t2,cursor:"pointer",fontFamily:FONT }}>Cancelar</button>
              </div>
            ) : (
              <div style={{ padding:"14px" }}>
                ...
                <div style={{ display:"flex",gap:8 }}>
                  <button onClick={()=>setShowAdd(false)} style={{ flex:1,background:C.s2,border:`1px solid ${C.s3}`,borderRadius:12,padding:"10px",fontSize:13,fontWeight:600,color:C.t2,cursor:"pointer",fontFamily:FONT }}>Cancelar</button>
                  <button className="pressable" onClick={handleAddCustom} style={{ flex:2,background:routine.color,border:"none",borderRadius:12,padding:"10px",fontSize:13,fontWeight:700,color:"#fff",cursor:"pointer",fontFamily:FONT,boxShadow:`0 4px 14px ${routine.color}50` }}>Añadir</button>
                </div>
              </div>
            )}
          </div>
          ...
          <button className="pressable" onClick={()=>{ setShowAdd(true); setAddMode("preset"); }}
            style={{ width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:8,background:"none",border:`1.5px dashed ${C.s4}`,borderRadius:18,padding:"14px",fontSize:13,fontWeight:700,color:C.t3,cursor:"pointer",fontFamily:FONT,marginBottom:10 }}>
        ...
        {doneCount===total&&total>0&&(
          <div className="anim-slideUp" style={{ textAlign:"center",padding:"28px 20px 12px" }}>
            ...
            <div style={{ fontSize:22,fontWeight:900,color:C.t1,letterSpacing:"-0.3px" }}>¡Rutina completada!</div>
            ...
            <button className="pressable" onClick={onBack} style={{ marginTop:16,background:C.s1,border:`1px solid ${C.s3}`,borderRadius:14,padding:"12px 32px",fontSize:14,fontWeight:700,color:C.t1,cursor:"pointer",fontFamily:FONT }}>Volver a rutinas</button>
          </div>
        )}
      </div>
    </div>
...
export default function App() {
  ...
  */
  ...
  const [modal,     setModal]     = useState(null);    // null | "startConfirm" | "routinePicker" | "lightbox"
  const [modalData, setModalData] = useState(null);
  const [activeRoutine, setActiveRoutine] = useState(null);

  // Seed initial history entry on mount
  useEffect(() => {
    window.history.replaceState({ screen:"tab", tab:"home", modal:null }, "");
  }, []);
